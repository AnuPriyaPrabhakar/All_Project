...
class ApplicationfromeService {
  ...
  getDeclarationForm = async (kycId: any) => {
    try {
        ...
        console.log('kycDeclaration:', response);
        return response;
    } catch (error) {
...
//   Directorslist = async (KycformData: CreateData[]) => {
//     try {
...
//     }
// };
// Directorslists = async (KycformDataa: CreateData[]) => {
//   try {
...
Directorslist = async (KycformData: NewPayload) => {
  try {
    console.log('Payload being sent to API FE:', KycformData);
      const response = await this.httpClientWrapper.kycpost('/api/v1/DirectorsList/CreateDirectorsList', KycformData);
      return response;
  } catch (error) {
      throw error;
...
uploadScoreDocument = async (
  ...
  try {
    const response = await this.httpClientWrapper.kycpostFormData(
      '/api/v1/ScoreDocument/files/upload',
      formData
    );
...
//   try {
//       const response = await this.httpClientWrapper.kycget(`/api/v1/DeclarationForm/${declarationId}`);
//       return response;
//   } catch (error) {
//       console.error('Error fetching kycDeclaration data:', error);
//       throw error;
