...
const BulkDataTable: React.FC = () => {
  ...
  const handleTableRowClick = async (criminalId: number, fileType: number, index: number, searchId: string, recId: string) => {
    ...
    if (fileType === 1) {
      ...
      // alert(`criminalId: ${id}, fileType: ${fileType}`); // Corrected alert syntax
  ...
  const handleRemarksSubmit = async () => {
    if (selectedRow) {
      ...
      try {
        ...
        setLoading(true);
        // alert(`criminalId: ${recId}, `); // Corrected alert syntax

        await hitdatalifecycleApiService.CreatLevelFlowcycle(hitrecordlifecyclePayload);
        // await fetchLevelpending(levelpending[0].searchId); // Assuming the ID to fetch is from levelpending
  ...
  //       levelpending.some(
  //         (record) =>
  //           `${record.searchId}-${record.recId}-${levelpending.indexOf(record)}` === selectedRow
  //       )
  //     ) {
  ...
  //         // Call the API to save the flow cycle
  //         await hitdatalifecycleApiService.CreatLevelFlowcycle(hitrecordlifecyclePayload);
  ...
  //         setSnackbarMessage("Saved successfully!");
  //         setOpenSnackbar(true);
  //         setSelectedAction(''); // Reset selected action (assuming "0" is the default)
  ...
  //         setIsRemarksDialogOpen(false);
  //       } catch (error) {
  //         console.error("Error while saving lifecycle flow:", error);
  //       } finally {
                                            ...
                                            {levelpending.map((record, index) => {
                                              ...
                                              return (
                                                <TableRow key={record.recId} sx={{ backgroundColor: index % 2 === 0 ? 'white' : '#d3d3d34f' }}>
                                                  ...
                                                  <TableCell>
                                                    <IconButton
                                                      sx={{ padding: '1px', zIndex: 998 }} // Ensure IconButton has lower z-index
