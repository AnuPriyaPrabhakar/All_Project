...
const SanTaskAssign = () => {
    ...
    //         const combined = sanDataResponse.map((item: BulkData) => {
    //             const taskAssign = assignedResponse.find((assignedItem: AssignedData) =>
    //                 assignedItem.isTaskAssigned === item.isTaskAssigned 
    //             );
    //             return {
    //                 ...item,
    //                 isTaskAssigned: taskAssign ? taskAssign.isTaskAssigned : 0
    //             };
    //         });
    ...
    const fetchData = async () => {
        try {
            // Fetch the San Bulk Data (isTaskAssigned = 0)
            const sanDataResponse: BulkData[] = await searchApi.getSearch();
            console.log("Fetched sanDataResponse:", sanDataResponse);

            // Fetch the Assigned Task Data (isTaskAssigned = 1)
            const assignedResponse: AssignedData[] = await searchApi.getAssignedData();
            console.log("Fetched assignedResponse:", assignedResponse);

            // Create a set of names from the assignedResponse where isTaskAssigned is 1
            const assignedNames = new Set(assignedResponse.map((assignedItem: AssignedData) => assignedItem.searchName));

            // Filter the sanDataResponse to exclude any items where the name exists in the assignedResponse
            const filteredSanData = sanDataResponse.filter(item => !assignedNames.has(item.name));

            // Filter the assignedResponse to exclude any items where the name exists in the sanDataResponse
            const filteredAssignedData = assignedResponse.filter(item => !sanDataResponse.some(sanItem => sanItem.name === item.searchName));
            ...
            // Update the state with the filtered combined data
            setCombinedData(combined);

            // Update assigned rows (if needed)
            setAssignedRows(combined.filter(item => item.isTaskAssigned === 1).map(item => item.id));
