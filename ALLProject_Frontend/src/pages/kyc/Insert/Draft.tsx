...
// import { AppFormData, CreateData, DeclarationFrom, GetData, GetDatas, kycForm as KycFormPayload, QuestionType } from '../../../data/services/kyc/applicationfrom/applicationfrome-payload';
...
//     answer: string;
//     questionId: string; // Assuming questionId is of type string, adjust as per your data
//     question: string;
//     ansTypeId: number; // Assuming ansTypeId is of type number, adjust as per your data
// }
...
//     const [pageDisabled, setPageDisabled] = useState(true); // Set initial state to true
//     const [editingFields, setEditingFields] = useState<number[]>([]); // State to track which fields are being edited
...
//                             </div>
//                             <img src={contactImage} alt="Contact" style={{ display: 'block', margin: '20px auto 0', maxWidth: '55%' }} />
//                             {/* <div style={{ textAlign: 'right', marginTop: '10px', position: 'absolute', bottom: '20px', right: '20px', fontSize: 'small' }}>
...
//         setPageNumber((prevPageNumber) => Math.min(prevPageNumber + 1, numPages!));
//     };
//     const [editing, setEditing] = useState(true); // Set editing to true by default
...
//     const [section, setSection] = useState('Letterhead');

//     const handleInputChange = (sectionName: any, event: any) => {
//         // Input change handling logic
//         // You may update the formData state here based on the sectionName and event
//     };
...
//         //     if (savedFormValues) {
//         //         setFormData(JSON.parse(savedFormValues));
//         //     }
//         // }
//     };
...
//                     <br></br>
//                     {/* <Grid item xs={12} style={{ float: 'inline-end', }}>
...
//                                                                                     key={answerIndex}
//                                                                                     value={answerData.answer}
//                                                                                     onChange={(e) => handleAnswerChange(pageIndex, answerIndex, e.target.value)}
...
import ponsunImage from '../../../assets/ponsun.png';
import ApplicationfromeService from '../../../data/services/kyc/applicationfrom/applicationfrome-api-service';
import { CreateData, DeclarationFrom, GetData, GetDatas, kycForm as KycFormPayload, QuestionType, UpdateAppFormData, UpdateApplicantFormDetailsData, UpdateApplicantFormDto } from '../../../data/services/kyc/applicationfrom/applicationfrome-payload';
import { LocalizationProvider } from '@mui/x-date-pickers/LocalizationProvider';
import { AdapterDayjs } from '@mui/x-date-pickers/AdapterDayjs';
import contactImage from '../../../assets/contact.png';
import { faDownload, faPlus, faTimes } from '@fortawesome/free-solid-svg-icons';
...
// interface kycAnswerData {
//     answer: string;
//     questionId: number; // Assuming questionId is of type string, adjust as per your data
//     question: string;
//     ansTypeId: number; // Assuming ansTypeId is of type number, adjust as per your data
// }
...
function Questionnaire({ handleInputChange }: SectionProps) {
    ...
    //             if (customerData && customerData.length > 0) {
    //                 const kycFormDto = customerData[0].kycFormDto;

    //                 if (
    //                     kycFormDto &&
    //                     kycFormDto.kycAnswerData &&
    //                     Array.isArray(kycFormDto.kycAnswerData)
    //                 ) {
    //                     setUpdateFormData({
    //                         applicantFormDto: {
    //                             id: kycFormDto.id,
    //                             name: kycFormDto.name,
    //                             numberOfPrint: 0, // You may need to update this if the actual data structure changes
    //                             applicantFormDetailsData: kycFormDto.kycAnswerData.map((data: kycAnswerData, index) => ({
    //                                 id: index, // Since there's no id in kycAnswerData, using index as id
    //                                 kycId: kycFormDto.id,
    //                                 accountTypeId: 0, // Placeholder, update as necessary
    //                                 applicationTypeId: 0, // Placeholder, update as necessary
    //                                 questionId: kycFormDto.id, // Adjust as necessary
    //                                 subQuestionId: kycFormDto.subQuestionId,
    //                                 ansTypeId: kycFormDto.ansTypeId,
    //                                 ansId: 0, // Placeholder, update as necessary
    //                                 isSubAnswer: "", // Placeholder, update as necessary
    //                                 answer: data.answer,
    //                                 score: data.score,
    //                                 uid: 0, // Placeholder, update as necessary
    //                             })),
    //                         },
    //                     });
    //                 } else {
    //                     console.error("Invalid structure in kycFormDto:", kycFormDto);
    //                     setErrors(["Invalid data structure"]);
    //                 }
    //             } else {
    //                 setErrors(["No data found"]);
    //             }
    ...
    //         fetchData(kycId);
    //     } else {
    //         console.error("No kycId provided");
    //     }
    // }, [kycId]);
    ...
    useEffect(() => {
        const responseId = sessionStorage.getItem('responseId');
        if (responseId) {
            const kycId = Number(responseId); // Convert to number if necessary
            if (!isNaN(kycId)) {
                // fetchData(kycId.toString()); // Pass kycId as string to fetchData
            } else {
    ...
    const handleAnswerChange = (questionIndex: number, fieldIndex: number, value: string | number) => {
        console.log('Before Update:', formData); // Log current formData before update
        ...
        setFormData(prevFormData => {
            ...
            console.log('After Update:', updatedFormData); // Log updated formData
            return updatedFormData;
        });
    };
    ...
    const handleEditSubmit = async () => {
        try {
            ...
            const updatedDetailsData: UpdateApplicantFormDetailsData[] = updateFormData.applicantFormDto.applicantFormDetailsData.map((data) => ({
                id: data.id || 0, // Use 0 for new entries
                kycId: data.kycId,
            ...
            const updatedFormData: UpdateAppFormData = {
                applicantFormDto: {
                    id: updateFormData.applicantFormDto.id || 0, // Use 0 for new form
    ...
    //         console.log('Before Submission:', updateFormData); // Log formData before submission
    
    //         const updatedFormData: UpdateAppFormData = {
    //             applicantFormDto: {
    //                 id: updateFormData.applicantFormDto.id,
    //                 name: updateFormData.applicantFormDto.name,
    //                 numberOfPrint: updateFormData.applicantFormDto.numberOfPrint,
    //                 applicantFormDetailsData : updateFormData.applicantFormDto.applicantFormDetailsData.map((data: UpdateApplicantFormDetailsData) => ({
    //                     id: data.id,
    //                     kycId: data.kycId,
    //                     accountTypeId: data.accountTypeId,
    //                     applicationTypeId: data.applicationTypeId,
    ...
    //                     ansTypeId: data.ansTypeId,
    //                     ansId: data.ansId,
    //                     isSubAnswer: data.isSubAnswer.toString(), // Convert to string if needed
    //                     answer: data.answer,
    //                     score: data.score,
    //                     uid: data.uid,
    //                 })),
    ...
    //         const response = await applicationfrome.ApllicationUpdate(responseIdNumber, updatedFormData);
                                                        ...
                                                        <TableRow key={index}>
                                                            ...
                                                            <TableCell>
                                                                {item.ansTypeId === 1 ? (
                                                                    ...
                                                                    <Select
                                                                        style={{ fontSize: 'small' }} // Adjust fontSize here
                                                                        fullWidth
                                                                        size='small'
                                                                        value={item.kycAnswerData && item.kycAnswerData.length > 0 ? item.kycAnswerData[0]?.answer : ''}
