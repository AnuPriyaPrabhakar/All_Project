...
const Declaration = (props: any) => {
    ...
    const handleSubmit = async (event: { preventDefault: () => void; }) => {
        ...
        try {
            if (props.declarationData.kycId === 0) {
                console.error('Invalid kycId');
                return;
            }
            const response = await applicationFormApiService.DeclarationForm(props.declarationData);
    ...
    useEffect(() => {
        if (kycId) {
            ...
            updateUiFromDbData(kycId);
        }
    }, [kycId]);
    

    // const updateUiFromDbData = async (declarationId: string) => {
    //     try {
    //         const declarationdata = await applicationFormApiService.getDeclarationFormById(declarationId);
    //         console.log('Declaraation data:', declarationdata);
    //         // props.handleInputChange('dbData', declarationdata);
    //         if (declarationdata.length > 0) {
    //             setDeclarationData(declarationdata[0]);
    //             props.handleInputChange('dbData', declarationdata[0]); // if needed
    //         }
    //         setShowSaveBtn(false);
    //         props.renderListOfBoardContent();
    //     }
    //     catch (error) {
    ...
    const updateUiFromDbData = async (kycId: string) => {
        try {
            ...
            if (declarationdata.length > 0) {
                setDeclarationData(declarationdata[0]);
                props.handleInputChange('dbData', declarationdata[0]); // Update the UI with the fetched data
            }
            setShowSaveBtn(false);
            props.renderListOfBoardContent();
        }
            ...
            <div id="pdfContent">
                <Paper elevation={10} style={{ marginBottom: '20px' }}>
                    <div style={{ position: 'relative', width: '100%', minHeight: '100%', padding: '20px' }}>
                        ...
                        <Typography variant="body1" paragraph>
                            ✓ We hereby confirm that <TextField variant="standard" size='small' inputProps={{ style: { textAlign: 'center' } }} style={{ textAlign: 'center' }} name="din" value={props.declarationData?.din} autoComplete='off' onChange={(event) => props.handleInputChange('application', event)} /> number of our | the company’s Director(s) is/are a “Politically Exposed Person (PEP)” or “close relative(s) of a PEP” or appear in the “list of terrorist individuals / entities” provided by RBI. In the event of our existing Director(s) is/are “PEP” or “close relative(s) of PEP” or appear in the list of “terrorist individuals / entities” provided by RBI, the details of same shall be furnished to NPCI on letter head.
                        </Typography>
                        <Typography variant="body1" paragraph>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            <div>
                                <Typography variant="body1" paragraph>
                                    Date: <TextField variant="standard" size='small' name="date" type="date" value={props.declarationData?.date} onChange={(event) => props.handleInputChange('application', event as ChangeEvent<HTMLInputElement>)} />
                                </Typography>
                                <Typography variant="body1" paragraph>
                                    Place: <TextField variant="standard" size='small' name="place" value={props.declarationData?.place} autoComplete='off' onChange={(event) => props.handleInputChange('application', event as ChangeEvent<HTMLInputElement>)} />
                                </Typography>
                            </div>
                            <div>
                                ...
                                <Typography variant="body1" paragraph align="right">
                                    <TextField name="authorizeName" size='small' value={props.declarationData?.authorizeName} autoComplete='off' onChange={(event) => props.handleInputChange('application', event as ChangeEvent<HTMLInputElement>)} placeholder="Name" />
                                </Typography>
                                <Typography variant="body1" paragraph align="right">
                                    <TextField name="authorizeDesignation" size='small' value={props.declarationData?.authorizeDesignation} autoComplete='off' onChange={(event) => props.handleInputChange('application', event as ChangeEvent<HTMLInputElement>)} placeholder="Designation" />
                                </Typography>
                                <Typography variant="body1" paragraph align="right">
                                    (Name & Designation)
                                </Typography>
